...
######################################
#TMUX_VERSION=3.3a
TMUX_VERSION="$(curl -qfsSL https://api.github.com/repos/tmux/tmux/releases/latest | jq -r '.tag_name')"
export TMUX_VERSION="$TMUX_VERSION"
#MUSL_VERSION=1.2.3
MUSL_VERSION="$(curl -qfsSL https://www.musl-libc.org/releases/ | grep -o 'href="[^"]*"' | sed 's/href="//' | sed 's/"$//' | sed 's/\/releases\/musl-//' | sed 's/\.tar\.gz//' | grep -E '^[0-9]+(\.[0-9]+)*$' | sort | tail -n 1)"
export MUSL_VERSION="$MUSL_VERSION"
#NCURSES_VERSION=6.3
NCURSES_VERSION="$(curl -qfsSL https://invisible-island.net/archives/ncurses/current/ | grep -o 'href="[^"]*"' | awk -F'"' '{print $2}' | grep '^ncurses.*\.tgz$' | awk -F'-' '{print $2}' | sort | tail -n 1)"
export NCURSES_VERSION="$NCURSES_VERSION"
LIBEVENT_VERSION=2.1.12
#LIBEVENT_VERSION="$(curl -qfsSL https://api.github.com/repos/libevent/libevent/tags | jq -r '.[0].name' | grep -o '[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*')"
export LIBEVENT_VERSION="$LIBEVENT_VERSION"
#UPX_VERSION=4.0.0
UPX_VERSION="$(curl -qfsSL https://api.github.com/repos/upx/upx/releases/latest | jq -r '.tag_name')"
...
#https://www.musl-libc.org/releases/musl-latest.tar.gz
MUSL_ARCHIVE="musl-latest.tar.gz"
